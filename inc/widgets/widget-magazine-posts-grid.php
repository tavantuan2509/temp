 ...
 */
class test_Magazine_Posts_Grid_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget.
		parent::__construct(
			'test-magazine-posts-grid', // ID.
			sprintf( esc_html__( 'Magazine Posts: Grid (%s)', 'test' ), wp_get_theme()->Name ), // Name.
			array(
				'classname' => 'test-magazine-posts-grid',
				'description' => esc_html__( 'Displays your posts from a selected category in a grid layout. Please use this widget ONLY in the Magazine Homepage widget area.', 'test' ),
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_test_magazine_posts_grid', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_test_magazine_posts_grid', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
				...
				<?php endif; ?>
						...
						<article id="post-<?php the_ID(); ?>" <?php post_class( 'medium-post clearfix' ); ?>>
							...
							<header class="entry-header">
								...
								<?php test_post_image( 'test-thumbnail-medium' ); ?>
	...
	function widget_title( $args, $settings ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			if ( $settings['category'] > 0 ) :
				...
				$link_title = sprintf( esc_html__( 'View all posts from category %s', 'test' ), get_cat_name( $settings['category'] ) );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'test' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php esc_html_e( 'Category:', 'test' ); ?></label><br/>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'layout' ); ?>"><?php esc_html_e( 'Grid Layout:', 'test' ); ?></label><br/>
			<select id="<?php echo $this->get_field_id( 'layout' ); ?>" name="<?php echo $this->get_field_name( 'layout' ); ?>">
				<option <?php selected( $settings['layout'], 'two-columns' ); ?> value="two-columns" ><?php esc_html_e( 'Two Columns Grid', 'test' ); ?></option>
				<option <?php selected( $settings['layout'], 'three-columns' ); ?> value="three-columns" ><?php esc_html_e( 'Three Columns Grid', 'test' ); ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'excerpt' ); ?>">
				...
				<?php esc_html_e( 'Display post excerpt', 'test' ); ?>
			</label>
		</p>
...
<?php
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_test_magazine_posts_grid', 'widget' );
 ...
 * Register Widget
 */
function test_register_magazine_posts_grid_widget() {

	register_widget( 'test_Magazine_Posts_Grid_Widget' );

}
add_action( 'widgets_init', 'test_register_magazine_posts_grid_widget' );
